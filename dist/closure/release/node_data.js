...
const assertionsModule = goog.require('incrementaldom.src.assertions');
...
class NodeData {
    ...
    getAttrsArr(length) {
        return this._attrsArr || (this._attrsArr = utilModule.createArray(length));
    }
}
/**
...
function importSingleNode(node, fallbackKey) {
    ...
    const nodeName = domUtilModule.isElement(node) ? node.localName : node.nodeName;
    const keyAttrName = globalModule.getKeyAttributeName();
    const keyAttr = domUtilModule.isElement(node) && keyAttrName != null
        ? node.getAttribute(keyAttrName)
        : null;
    const key = domUtilModule.isElement(node) ? keyAttr || fallbackKey : null;
    const data = initData(node, nodeName, key);
    if (domUtilModule.isElement(node)) {
...
function getKey(node) {
    assertModule.assert(node["__incrementalDOMData"]);
