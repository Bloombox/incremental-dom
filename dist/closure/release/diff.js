...
const changesModule = goog.require('incrementaldom.src.changes');
const util_1 = goog.require('incrementaldom.src.util');
...
function calculateDiff(prev, next, updateCtx, updateFn) {
    ...
    for (; i < next.length; i += 2) {
        ...
        if (isNew || prev[i + 1] !== value) {
            prev[i + 1] = value;
            changesModule.queueChange(updateFn, updateCtx, name, value);
        }
    }
    ...
    if (i < next.length || i < prev.length) {
        ...
        for (const name in prevValuesMap) {
            changesModule.queueChange(updateFn, updateCtx, name, undefined);
            delete prevValuesMap[name];
        }
    }
    flush();
